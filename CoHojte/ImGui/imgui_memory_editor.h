...
struct MemoryEditor
{
    ...
    void DrawWindow(const char* title, void* mem_data, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        ...
        if (ImGui::IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows) && ImGui::IsMouseReleased(ImGuiMouseButton_Right))
            ImGui::OpenPopup(make_string("context").c_str());
    ...
    void DrawContents(void* mem_data_void, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        ...
        ImGui::BeginChild(make_string("##scrolling").c_str(), ImVec2(0, -footer_height), false, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoNav);
            ...
            for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
            {
                ...
                for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
                {
                    ...
                    if (DataEditingAddr == addr)
                    {
                        ...
                        if (ImGui::InputText(make_string("##data").c_str(), DataInputBuf, IM_ARRAYSIZE(DataInputBuf), flags, UserData::Callback, &user_data))
    ...
    void DrawOptionsLine(const Sizes& s, void* mem_data, size_t mem_size, size_t base_display_addr)
    {
        ...
        // Options menu
        if (ImGui::Button(make_string("Options").c_str()))
            ImGui::OpenPopup(make_string("context").c_str());
        if (ImGui::BeginPopup(make_string("context").c_str()))
        {
            ImGui::SetNextItemWidth(s.GlyphWidth * 7 + style.FramePadding.x * 2.0f);
            if (ImGui::DragInt(make_string("##cols").c_str(), &Cols, 0.2f, 4, 32, "%d cols")) { ContentsWidthChanged = true; if (Cols < 1) Cols = 1; }
            ImGui::Checkbox(make_string("Show Data Preview").c_str(), &OptShowDataPreview);
            ImGui::Checkbox(make_string("Show HexII").c_str(), &OptShowHexII);
            if (ImGui::Checkbox(make_string("Show Ascii").c_str(), &OptShowAscii)) { ContentsWidthChanged = true; }
            ImGui::Checkbox(make_string("Grey out zeroes").c_str(), &OptGreyOutZeroes);
            ImGui::Checkbox(make_string("Uppercase Hex").c_str(), &OptUpperCaseHex);
        ...
        if (ImGui::InputText(make_string("##addr").c_str(), AddrInputBuf, IM_ARRAYSIZE(AddrInputBuf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_EnterReturnsTrue))
        {
            size_t goto_addr;
        ...
        if (GotoAddr != (size_t)-1)
        {
            ...
            {
                ImGui::BeginChild(make_string("##scrolling").c_str());
    ...
    void DrawPreviewLine(const Sizes& s, void* mem_data_void, size_t mem_size, size_t base_display_addr)
    {
        ...
        ImGui::Text(make_string("Preview as:").c_str());
        ImGui::SameLine();
        ImGui::SetNextItemWidth((s.GlyphWidth * 10.0f) + style.FramePadding.x * 2.0f + style.ItemInnerSpacing.x);
        if (ImGui::BeginCombo(make_string("##combo_type").c_str(), DataTypeGetDesc(PreviewDataType), ImGuiComboFlags_HeightLargest))
        {
        ...
        ImGui::Combo(make_string("##combo_endianess").c_str(), &PreviewEndianess, "LE\0BE\0\0");
        ...
        ImGui::Text(make_string("Dec").c_str()); ImGui::SameLine(x); ImGui::TextUnformatted(has_value ? buf : "N/A");
        if (has_value)
            DrawPreviewData(DataPreviewAddr, mem_data, mem_size, PreviewDataType, DataFormat_Hex, buf, (size_t)IM_ARRAYSIZE(buf));
        if(ImGui::Selectable(make_string("Hex").c_str(),false, ImGuiSelectableFlags_AllowDoubleClick))
            ImGui::SetClipboardText(toHexString(buf).c_str());

        ImGui::SameLine(x); ImGui::TextUnformatted(has_value ? buf : "N/A");
        if (has_value)
            DrawPreviewData(DataPreviewAddr, mem_data, mem_size, PreviewDataType, DataFormat_Bin, buf, (size_t)IM_ARRAYSIZE(buf));
        buf[IM_ARRAYSIZE(buf) - 1] = 0;
        ImGui::Text(make_string("Bin").c_str()); ImGui::SameLine(x); ImGui::TextUnformatted(has_value ? buf : "N/A");
    }
